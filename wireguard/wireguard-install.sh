...
EOF
	...
	cat << EOF > /mnt/d/vpnbot/wireguard/"$client".conf
[Interface]
...
EOF
	...
	echo
	qrencode -t ANSI256UTF8 < /mnt/d/vpnbot/wireguard/"$client.conf"
	echo -e '\xE2\x86\x91 That is a QR code containing the client configuration.'
	echo
	echo "Finished!"
	echo
	echo "The client configuration is available in:" /mnt/d/vpnbot/wireguard/"$client.conf"
	...
	case "$option" in
		1)
			...
			echo
			echo "$client added. Configuration available in:" /mnt/d/vpnbot/wireguard"$client.conf"
			exit
		;;
		2)
